...
export const CompaniesTab: React.FC<CompaniesTabProps> = ({ onSelectCompany }) => {
  // Filter out bankrupt companies (Delisting)
  const companies = useGameStore(useShallow(s => s.gameState.companies.filter(c => !c.isBankrupt)));
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      <div>
        ...
        {companies.length === 0 ? (
            <div className="text-center py-12 bg-stone-900 rounded-xl border border-stone-800 border-dashed text-stone-500">
                暂无上市企业 (No Active Listings)
            </div>
        ) : (
